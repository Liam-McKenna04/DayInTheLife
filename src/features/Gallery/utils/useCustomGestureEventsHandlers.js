...
export const useCustomGestureEventsHandlers = () => {
  ...
  const handleOnStart = useWorkletCallback(
    (_, { translationY }, context) => {
      // cancel current animation - not currently working
      stopAnimation();
  ...
  const handleOnActive = useWorkletCallback(
    (source, { translationY }, context) => {
      gestureTranslationY.value = translationY;
      ...
      console.log();
      animatedPosition.value = draggedPosition;
    },
    [
      enableOverDrag,
      enablePanDownToClose,
