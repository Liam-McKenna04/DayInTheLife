...
export const isFirstStartup = async () => {
  const mainDIR = await FileSystem.getInfoAsync(
    FileSystem.documentDirectory + "DayInTheLife"
  );
...
export const FinalizeVideo = async (VideoURIEnding) => {
  await FileSystem.moveAsync({
    from: FileSystem.documentDirectory + "DayInTheLife/Today/" + VideoURIEnding,
    to: FileSystem.documentDirectory + "DayInTheLife/Days/" + VideoURIEnding,
  });
...
export const GetVideoSegments = async () => {
  const todaylist = await FileSystem.readDirectoryAsync(
  ...
  if (todaylist == null || todaylist == [] || todaylist == undefined) {
    ...
    return todaylist.map(
      (item) => FileSystem.documentDirectory + "DayInTheLife/Today/" + item
    );
  }
};
